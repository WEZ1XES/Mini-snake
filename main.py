...
def main():
    ...
    root = Tk()
    root.title('Змейка by Prosto Alex')
    root.geometry('800x600+150+150')

    frame = Frame(root, width=740, height=90, bg='#f2ffe0')
    frame.place(x=30, y=5)
    text = Label(root, text='''Игра Змейка от Prosto Alex. Правила игры : Нельзя вылезать за поля, управляется стрелочками.
     Змейка кушает плоды и становится быстрее. Скорость можно регулировать  на клавиши + и -
''',
                  bg='#f2ffe0', width=79)
